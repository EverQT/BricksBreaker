...
void APaddle::MoveHorizontal(float AxisValue)
{
	...
	AddMovementInput(FVector(AxisValue, 0.0f, NewLocation.Z < 25), 1.0f, false);
...
void APaddle::MoveBrick(float BSAxisValue)
{
	FVector NewLocation = GetActorLocation();
	AddMovementInput(FVector(0.0f, 0.0f, BSAxisValue), 1.0f, false);
}
